...
import 'package:senmi/senmi_package_screens/features/customer/customer_history/customer_history_screen.dart';
...
class _EditPackageScreenState extends State<EditPackageScreen> {
  ...
  void initState() {
    ...
    pickupController.text = widget.package["pickup_address"] ?? "";

    deliveryController.text = widget.package["delivery_address"] ?? "";

    receiverNameController.text = widget.package["receiver_name"] ?? "";

    receiverPhoneController.text = widget.package["receiver_phone"] ?? "";

    pickupLocation = LatLng(
      double.parse(widget.package["pickup_lat"].toString()),
      double.parse(widget.package["pickup_lng"].toString()),
    );

    deliveryLocation = LatLng(
      double.parse(widget.package["delivery_lat"].toString()),
      double.parse(widget.package["delivery_lng"].toString()),
    );

    estimatedPrice = double.tryParse(widget.package["price"].toString());
  ...
  Future<void> _updatePackage() async {
    ...
    try {
      ...
      final res = await ApiService.editPackage(
        widget.package["package_id"],
        payload,
      );

      if (!mounted) return;

      if (res["package_id"] != null || res["success"] == true) {
        ScaffoldMessenger.of(context).showSnackBar(
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(res["error"] ?? "Unable to update package")),
        );
      }
    } catch (e) {
      if (!mounted) return;
